<template>
  <div class="loginContainer">
    <head-top :head-title="'忘记密码'" goBack="true">
    </head-top>
...
<script>
  ...
  import {getForgetVerifyCode,verifyCheckCode} from'../../service/SmsService';
    ...
    methods: {
      ...
      async getVerifyCode(){
        if (this.rightPhoneNumber) {
          ...
          //发送短信验证码
          try{
            let res = await getForgetVerifyCode(this.phoneNumber);
            this.showAlert = true;
      ...
      async next(){
        if(this.rightMobileCode){
          try{
            ...
            this.$router.push({name:'xdjForgetSetPwd',params: {mobile:this.phoneNumber}});
          }catch(e){
            this.showAlert = true;
            this.alertText = e.message;
          }
